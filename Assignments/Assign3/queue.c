...
int main()
{
  ...
  //CarPtr stackPtr;
  //stackPtr = NULL;
  CarPtr headPtr = NULL; // add code to initialize headPtr
  CarPtr tailPtr = NULL; // add code to initialize tailPtr
  ...
  while (!exit)
  {
    ...
    case 4:
      ...
      if (!isEmpty(headPtr))
      {
        //topOfCarStack(headPtr);
        frontOfQueue(headPtr);
        printf("Do you want to delete car from Queue? (Y/N) ");
        fgets(input, sizeof(input), stdin);
        input[(strcspn(input, "\n"))] = '\0';
        choice = input[0];
